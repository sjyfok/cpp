...
int CMainWindow::OnCreate(LPCREATESTRUCT lpcs)
{
	...
	m_fontMain.CreatePointFont(80, _T("MS Scans Serif"));
	CClientDC dc(this);
	...
	m_wndLBTitle.Create(_T("Typefaces"), WS_CHILD | WS_VISIBLE | SS_LEFT,
		rect, this);
	rect.SetRect(m_cxChar * 2, m_cyChar * 2, m_cxChar * 48, m_cyChar * 18);
	...
	rect.SetRect(m_cxChar * 4, m_cyChar * 22, m_cxChar * 64,
		(m_cyChar * 99) / 4);
	m_wndSampleText.Create(_T(""), WS_CHILD | WS_VISIBLE | SS_CENTER, rect, this, IDC_SAMPLE);
...
void CMainWindow::OnSelChange()
{
	...
	if (nIndex == LB_ERR)
	{
		...
		m_wndSampleText.SetWindowTextA(_T(""));
	}
	else
	{
		...
		m_wndSampleText.SetWindowTextA(_T("AaBbCcDdEeFfGg"));
	}
}
